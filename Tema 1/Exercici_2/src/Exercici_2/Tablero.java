...
public class Tablero {
    ...
    private String[] barcosTXT= new String[5];
    private Random r;
    ...
    public Tablero() throws InterruptedException {
        ...
        for (int tamany : tamanys) {            
            colocar(tamany, contadorBarco++);
        }

    }

    public Tablero(String[] listadoBarcos){
        elTablero = new int[TAM][TAM];
      
        for (String barcoTXT : listadoBarcos) {  
            var barco = barcoTXT.split(";");
            int nBoat = Integer.parseInt(barco[0]);
            int tamany = Integer.parseInt(barco[1]);
            int orientacion = Integer.parseInt(barco[2]);
            int fila = Integer.parseInt(barco[3]);
            int columna = Integer.parseInt(barco[4]);     
            colocar(tamany, nBoat, orientacion, fila, columna);
        }
    }
    
    private void setBarcosTXT(int boat, int dimension, int direccion, int fila, int columna){
        this.barcosTXT[boat]=boat+";"+dimension+";"+direccion+";"+fila+";"+columna;    
    }
    
    public String[] getBarcosTXT(){
        return this.barcosTXT;
    }
    
    private void colocar(int tamany, int nBoat) {
        ...
        while (!colocat) {
            ...
            if (capBarco(x, y, ori, tamany)) {
                ...
                if (ori == 0) // hori
                {
                    setBarcosTXT(nBoat,tamany,0,y,x);
                ...
                {
                    setBarcosTXT(nBoat,tamany,1,y,x);                 
    ...
    private void colocar(int tamany, int nBoat, int orientacion, int fila, int columna) {
        // sorteamos la orientación del barco (H o V)
